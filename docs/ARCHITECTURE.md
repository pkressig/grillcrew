...
# Read-only OneDrive ingestion boundary

Der Downloader akzeptiert ausschliesslich HTTPS auf `1drv.ms`, `onedrive.live.com` oder `*.sharepoint.com`, verbietet Credentials/abweichende Ports, validiert alle DNS-Ergebnisse als global und bindet die tatsächliche TCP-Verbindung an eine geprüfte Adresse; TLS prüft weiterhin den ursprünglichen Hostnamen/SNI. Jeder Redirect wird neu geprüft. Downloads sind auf 10 MiB sowie begrenzte ZIP-Einträge/Entpackgrösse beschränkt und verwenden nur GET. Die Anwendung besitzt keine Microsoft-Credentials oder Schreibscopes. Render/Vercel stellen keinen persistenten Worker bereit; ein externer Produktions-Cron ruft `POST /api/cron/onedrive-sync/run-due` mit `Authorization: Bearer $ONEDRIVE_CRON_TOKEN` auf. Das mindestens 32 Zeichen lange Token existiert nur als Deployment-Umgebungsvariable. Tenant-lokale tägliche und Inhalts-Claims verhindern parallele Doppelläufe. `RUNNING` ist ein 15-minütiger Lease: ein konkurrierender Aufruf sperrt und beendet nur einen abgelaufenen Claim, bevor er ihn neu beansprucht; lebende Claims bleiben dedupliziert. Fehlgeschlagene oder abgelaufene Claims werden für Wiederholung freigegeben. Ein Lauf erzeugt höchstens einen `STAGED`-Import.
