...
# OneDrive read-only synchronization

`OneDriveSyncConfig` ist pro Organisation eindeutig und verweist auf das Ziel-Vereinsjahr sowie den zuletzt konfigurierenden ADMIN als technischen Auslöser geplanter Staging-Imports. Es enthält den normalisierten Quelllink, Aktivierung, lokale Tageszeit sowie Start-/Enddatum und den Zukunftsfilter. `OneDriveSyncRun` protokolliert Auslöser, effektiven Datumsbereich, Dateiname, SHA-256, Status/Fehler, Zeilenzahl, Vergleichszusammenfassung und optional das erzeugte `ImportBatch`. Ein partieller eindeutiger Hash-/Datumsindex für `RUNNING` und `SUCCESS` beansprucht identischen Inhalt vor der Erstellung eines Staging-Batches; `FAILED` liegt ausserhalb des Index und bleibt wiederholbar. Eine zweite Eindeutigkeit beansprucht pro Konfiguration und lokalem Tag höchstens einen geplanten Lauf. Zugangsdaten werden nicht persistiert.
