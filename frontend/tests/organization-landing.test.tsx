...
const renderPage = (shiftType: "GRILL" | "KIOSK" = "GRILL") =>
  render(
  ...
  describe("account modal", () => {
    ...
    it("links Registrieren directly to the dedicated registration page", async () => {
      ...
      expect(await screen.findByRole("link", { name: "Registrieren" })).toHaveAttribute(
        "href",
        "/register?org=example&area=grill",
      );
    });
  });

  describe("shift type variants", () => {
    beforeEach(() => {
      mockedFetch.mockResolvedValue({ events: [] });
    });

    it("fetches the Grill plan and shows the Grill heading", async () => {
      renderPage("GRILL");
      await screen.findByRole("heading", { name: "Noch keine Einsätze" });
      expect(mockedFetch).toHaveBeenCalledWith("example", "GRILL", expect.any(AbortSignal));
      expect(screen.getByText("Grill Helfer Einsatzplan")).toBeInTheDocument();
    });

    it("fetches the Kiosk plan and shows the Kiosk heading", async () => {
      renderPage("KIOSK");
      await screen.findByRole("heading", { name: "Noch keine Einsätze" });
      expect(mockedFetch).toHaveBeenCalledWith("example", "KIOSK", expect.any(AbortSignal));
      expect(screen.getByText("Kiosk Helfer Einsatzplan")).toBeInTheDocument();
    });

    it("links Registrieren with the Kiosk area so registration returns to the right page", async () => {
      renderPage("KIOSK");
      expect(await screen.findByRole("link", { name: "Registrieren" })).toHaveAttribute(
        "href",
