...
describe("planning admin", () => {
  ...
  it("groups events under one heading per day, and lists every matching event with no pagination", async () => {
    const sameDayEvent = { ...planningEvent, id: "event-same-day", title: "Gleicher Tag" };
    const laterEvent = {
      ...planningEvent,
      id: "event-later",
      title: "Herbstmarkt",
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      if (
        String(input).includes("event-same-day/shifts") ||
        String(input).includes("event-later/shifts")
      )
        return Response.json([]);
      return base(input, init);
    });
    ...
    const list = await screen.findByRole("list", { name: "Agenda Herbstrunde" });
    // Sommerfest and Gleicher Tag share 2026-09-12: exactly one day heading for
    // both, then a separate heading for Herbstmarkt's later date — no
    // "Datumsnavigation"/Zurück/Weiter pagination gates any of this.
    expect(within(list).getAllByRole("heading", { level: 5 })).toHaveLength(2);
    expect(within(list).getByText("Sommerfest")).toBeInTheDocument();
    expect(within(list).getByText("Gleicher Tag")).toBeInTheDocument();
    expect(within(list).getByText("Herbstmarkt")).toBeInTheDocument();
    expect(screen.queryByRole("navigation", { name: "Datumsnavigation" })).not.toBeInTheDocument();
  ...
  it("does not delete a shift when confirmation is declined", async () => {
    const confirmMock = vi.fn(() => false);
    vi.stubGlobal("confirm", confirmMock);
    const fetchMock = renderAdmin("ADMIN", planningFetch("ADMIN", false, [season], true, true));
    await screen.findByText("Sommerfest");
    ...
    fireEvent.click(
      screen.getByRole("button", { name: `Einsatz ${shiftTime} für Sommerfest löschen` }),
    );

    expect(confirmMock).toHaveBeenCalledWith(
      `Einsatz ${shiftTime} für "Sommerfest" endgültig löschen?`,
    );
    expect(fetchMock.mock.calls.some(([, init]) => init?.method === "DELETE")).toBe(false);
  });

  it("deletes a shift after confirming and refreshes the list", async () => {
    vi.stubGlobal(
      "confirm",
      vi.fn(() => true),
    );
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        expect.stringMatching(/\/shifts\/shift-1$/),
        expect.objectContaining({ method: "DELETE", credentials: "include" }),
      ),
    );
    expect(await screen.findByText("Der Einsatz wurde endgültig gelöscht.")).toBeInTheDocument();
  });
